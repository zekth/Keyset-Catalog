<template>
  <svg>
    <g class="keycap U15">
      ...
      />
      <path
        style="transform:scaleX(1.75)translate(-51px, 0px);"
        :fill="data.colors.background"
        class
      ...
      <g
        v-if="!data.legendOverriden && data.key === 'KC_TAB'"
        transform="translate(120,110) scale(15)"
      >
        <tab :color="data.colors.legend" />
      </g>
      <g
        v-if="!data.legendOverriden && data.key === 'KC_BSPC'"
        transform="translate(-25,100) scale(15)"
      >
        <backspace15 :color="data.colors.legend" />
      </g>
